...
const PagosPage = () => {
    ...
    const getEstado = (fechaVenc) => {
        ...
        if (diff <= 3 && diff >= 0) return { texto: 'Por vencer', color: 'bg-yellow-300' };
        if (diff < 0) return { texto: 'Vencido', color: 'bg-red-400' };
        return { texto: 'Al día', color: 'bg-green-400' };
    };
                ...
                <div className="max-w-5xl mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {alumnos.map(alumno => {
                        ...
                        return (
                            <div
                                key={alumno._id}
                                className="bg-neutral-900 rounded-2xl shadow-sm p-5 flex flex-col justify-between transition-all hover:shadow-md hover:scale-[1.02] duration-300 border border-neutral-800"
                            >
                                <div className="flex items-center space-x-4">
                                    ...
                                    />
                                    <div>
                                        <h3 className="text-lg font-semibold text-neutral-100">{alumno.nombre} {alumno.apellido}</h3>
                                ...
                                <div className="mt-4 flex justify-between items-center">
                                    ...
                                    <button
                                        ...
                                        className="bg-orange-400 hover:bg-orange-500 text-white px-4 py-1 rounded-lg text-sm transition-all shadow-sm"
                                    >
                                        Marcar Pago
                                    </button>
                                </div>
